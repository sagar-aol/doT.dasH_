...
const updateDisplay = () => {
  if (isKeyboardMode) {
    ...
    ui.currentMorseBuffer.innerText = "";
  } else {
    ui.mainOutput.innerText = translatedMessage === "" ? "Awaiting input..." : translatedMessage;
...
// --- Calculator Inputs ---
ui.btnDot.addEventListener("click", () => {
  triggerFeedback("dot", ui.btnDot, true);
  currentMorseChar += ".";
  updateDisplay();
});
ui.btnDash.addEventListener("click", () => {
  triggerFeedback("dash", ui.btnDash, true);
  currentMorseChar += "-";
  updateDisplay();
});
ui.btnSpace.addEventListener("click", () => {
  triggerFeedback("dot", ui.btnSpace, false);
  if (currentMorseChar !== "") {
    translatedMessage += REVERSE_DICT[currentMorseChar] || "?";
    currentMorseChar = "";
  } else {
    translatedMessage += " ";
  }
...
const deleteAction = () => {
  ...
  updateDisplay();
  triggerFeedback("dot", ui.btnDel, false);
};
...
const startDelete = (e) => {
  e.preventDefault();
  deleteAction();
  deleteTimer = setTimeout(deleteWordAction, 600);
};
const endDelete = () => clearTimeout(deleteTimer);
ui.btnDel.addEventListener("mousedown", startDelete);
ui.btnDel.addEventListener("touchstart", startDelete, { passive: false });
ui.btnDel.addEventListener("mouseup", endDelete);
ui.btnDel.addEventListener("mouseleave", endDelete);
ui.btnDel.addEventListener("touchend", endDelete);
...
ui.btnReset.addEventListener("click", () => {
  ...
  ui.textInput.value = "";
  updateDisplay();
  triggerFeedback("dash", ui.btnReset, false);
});
ui.btnSend.addEventListener("click", () => {
  ...
  ui.mainOutput.style.backgroundColor = "var(--accent-1)";
  ui.mainOutput.style.color = "#1e1e24";
  setTimeout(() => {
    ui.mainOutput.style.backgroundColor = "var(--bg-secondary)";
    ui.mainOutput.style.color = "var(--text-primary)";
  }, 200);
...
const populateGrid = (dict, targetGrid) => {
  Object.entries(dict).forEach(([char, code]) => {
    ...
    item.className = "guide-item focus-ring";
    ...
    const copyAction = (e) => {
      navigator.clipboard.writeText(code).then(() => {
        ...
        gsap.fromTo(
          tooltip,
          { y: 10, opacity: 0, scale: 0.8, xPercent: -50 },
          {
            y: -5,
            opacity: 1,
            scale: 1,
        ...
        item.style.backgroundColor = "var(--accent-1)";
...
ui.btnExportTxt.addEventListener("click", () => {
  let textToExport = translatedMessage;
  if (!textToExport) return;
  const a = document.createElement("a");
  a.href = URL.createObjectURL(new Blob([textToExport], { type: "text/plain" }));
  a.download = "doT.dasH_Export.txt";
  a.click();
});
ui.btnExportCsv.addEventListener("click", () => {
  ...
  a.download = "doT.dasH_Export.csv";
  a.click();
});
