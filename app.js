...
const ui = {
  ...
  gridLetters: document.getElementById("guide-grid-letters"), gridNumbers: document.getElementById("guide-grid-numbers"),
  gridSymbols: document.getElementById("guide-grid-symbols"), gridSpecial: document.getElementById("guide-grid-special"),
...
let isSoundEnabled = false; let isVibeEnabled = true; let cropperInstance = null;
...
const triggerFeedback = (type, btnElement, playSound = false) => {
  ...
  if (btnElement && isVibeEnabled) {
    btnElement.classList.add("visual-shake"); btnElement.style.animationDuration = `${baseDuration}ms`;
    setTimeout(() => btnElement.classList.remove("visual-shake"), baseDuration);
  }
};

ui.btnKeyboardToggle.addEventListener("click", () => {
  isKeyboardMode = !isKeyboardMode; ui.btnKeyboardToggle.setAttribute("aria-pressed", isKeyboardMode);
  currentMorseChar = ""; translatedMessage = ""; ui.textInput.value = ""; updateDisplay(); triggerFeedback("dot", ui.btnKeyboardToggle, false);

  if (isKeyboardMode) {
    ui.btnKeyboardToggle.classList.add("active-keyboard-btn");
    gsap.to(ui.calcGrid, { autoAlpha: 0, y: 20, duration: 0.2, onComplete: () => {
        gsap.fromTo(ui.keyboardGrid, { autoAlpha: 0, y: -20 }, { autoAlpha: 1, y: 0, duration: 0.3, ease: "back.out(1.7)", onComplete: () => ui.textInput.focus() });
      }
    });
  } else {
    ui.btnKeyboardToggle.classList.remove("active-keyboard-btn");
    gsap.to(ui.keyboardGrid, { autoAlpha: 0, y: -20, duration: 0.2, onComplete: () => {
        gsap.fromTo(ui.calcGrid, { autoAlpha: 0, y: 20 }, { autoAlpha: 1, y: 0, duration: 0.3, ease: "back.out(1.7)" });
      }
    });
  }
});
...
const deleteAction = () => {
  ...
  updateDisplay(); triggerFeedback("dot", ui.btnDel, false);
};
...
const populateGrid = (dict, targetGrid) => {
  Object.entries(dict).forEach(([char, code]) => {
    ...
    item.className = "guide-item focus-ring"; item.setAttribute("tabindex", "0"); item.setAttribute("role", "button");
    ...
    const copyAction = (e) => {
      navigator.clipboard.writeText(code).then(() => {
        ...
        gsap.fromTo(tooltip, { y: 10, opacity: 0, scale: 0.8, xPercent: -50 }, {
...
populateGrid(DICT_LETTERS, ui.gridLetters); populateGrid(DICT_NUMBERS, ui.gridNumbers);
populateGrid(DICT_SYMBOLS, ui.gridSymbols); populateGrid(DICT_SPECIAL, ui.gridSpecial);
...
ui.btnExportTxt.addEventListener("click", () => {
  ...
  const url = URL.createObjectURL(new Blob([textToExport], { type: "text/plain" }));
  const a = document.createElement("a"); a.href = url; a.download = "doT.dasH_Export.txt"; a.click();
...
ui.btnExportCsv.addEventListener("click", () => {
  ...
  const a = document.createElement("a"); a.href = url; a.download = "doT.dasH_Export.csv"; a.click();
  setTimeout(() => URL.revokeObjectURL(url), 100);
});